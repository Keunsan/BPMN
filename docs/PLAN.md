...
> 구현 시점 미정 — 아키텍처 방향만 확정. 당장 작업하지 않음.

### L3/L4 ↔ 조직 매핑: 역할 마스터 경유 (2026-06-23)

**현재(임시)**: L3 또는 L4 프로세스 노드에 법인·사업부 등 **조직코드를 직접** 매핑 (Scope/Variant 등).

**목표 구조**:

```
조직코드 ──► 역할마스터(역할코드) ──► L3 또는 L4
```

1. **역할 마스터**에 역할코드 ↔ 조직코드 매핑을 둔다.
2. L3/L4에는 조직코드가 아닌 **역할코드**만 매핑한다.
3. 조직 관점 조회·영향도·RACI는 역할을 통해 간접 연결된다.

**구현 시 고려**: `role`·`organization`·`task_role_mapping`(또는 프로세스-역할 매핑) 스키마 정리, 기존 Scope/Variant·조직 필터 마이그레이션, 운영 지식그래프 엣지(`PERFORMED_BY` 등) 정합.
